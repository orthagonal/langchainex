defmodule PricingChainTest do
  @moduledoc """
  A Pricing Chain will attempt to run a query and vote on the best answer.
  """
  use ExUnit.Case
  ...
  # get the result for that request, get the price (which may mean scraping and asking for the price)
  # then ask the models to vote on which one is the best model

  # quorum modelling

  "Who's Got The Best AI Language Model, according to the AI Language Models"
  with_context "https://www.goose.ai/pricing" do
  with_context ""
    ask %LangChain.Providers.GooseAi.LanguageModel{} "
    Given this price sheet what is the price for this GooseAi request:
    <%= request %>
    "
  end
...
elixir <-> natural language



Elixir Prompt -> (execution of tasks) -> Elixir Answer

NL Prompt -> NL Answer (because of model execution)

NL Answer-> Elixir Prompt (because of parsing of NL Answer)

Elixir Prompt -> Elixir Answer (because of execution of Elixir)




prompt = "Write an Elixir Ecto query to handle the following request: "

vote("What is the best Provider for this request: 'What is the tallest mountain, expressed in meters?",
  models: [
