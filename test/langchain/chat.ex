defmodule LangChain.ChatTest do
  ...
  def create_chat() do
    ...
    ai_prompt = %LangChain.PromptTemplate{
      template: "I'm an AI named <%= ai_name %> and <%= spell %> is like <%= another_spell %> or <%= spell_three %> except better",
  ...
  test "Test serialize" do
    ...
    assert chatSerialized = %{
      ...
      prompt_messages: ["{\"prompt\":{\"input_variables\":[\"context\"],\"partial_variables\":{},\"src\":\"user\",\"template\":\"Here's some context: {context}\"}}",
    ...
    chatDeserialized = Enum.map(chatSerialized.prompt_messages, fn item ->
      Jason.decode!(item)
    end)
